...
tf.flags.DEFINE_float('dev_sample_percent', 0.7, 'Percentage of the training data to use for validation')
...
##Modeal Hparams
tf.flags.DEFINE_integer('embedding', 128, 'dimensonality of the character embedding')
tf.flags.DEFINE_string('filter_size', '3,4,5', 'filter size')
tf.flags.DEFINE_integer('number_filters', 128, 'Number of filters')
tf.flags.DEFINE_float('droupout', 0.5, 'Droup out')
tf.flags.DEFINE_float('L2_reg_lamabda', 0.0, 'L2')
...
tf.flags.DEFINE_integer('num_checkpoints', 5, "num_checkpoints")
...
print(np.shape(shuffle_indices))
x_shuffled = x[shuffle_indices]
y_shuffled = y[shuffle_indices]

dev_sample_index = -1 * int(FLAGS.dev_sample_percent * float(len(y)))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN(
            ...
            l2_reg_lambda=FLAGS.L2_reg_lamabda
        )
